...
<link rel="stylesheet"
	href="/robot/resources/css/backend/paper-list.css"
			...
			<div id="pad-wrapper" class="users-list">
				<div class="row-fluid header">
					...
					<div class="span10 pull-right">
						...
						<a href="/robot/backend/paper/add" class="btn-flat success pull-right"> <span>&#43;</span>
							添加论文
						</a>
					</div>
				</div>
			
				<h5>系统共有${pages.totalCount}篇论文</h5>
				<!-- Users table -->
				...
				<c:otherwise>
					<li><a href="/robot/backend/paper/papers?page=${pages.currentPage-1}&pageSize=${pages.pageSize}">&#8249;</a></li>
				</c:otherwise>
				</c:choose>
				...
				<c:choose>
					<c:when test="${pages.totalPage lt 5}">
						<c:forEach var="item" varStatus="status" begin="1" end="${pages.totalPage}">
						  	<li ><a <c:if test="${pages.currentPage eq status.index}">class="active"</c:if> href="/robot/backend/paper/papers?page=${status.index}&pageSize=${pages.pageSize}">${status.index}</a></li>
						</c:forEach>
					</c:when>
					<c:otherwise>
